...
async function getLignesActives() {
  // Route publique /api/lines/active — pas besoin d'auth
  const json = await fetch(API_BASE + '/api/lines/active').then(r => r.json());
  const list = json?.data || json?.lines || [];
  return list.map(l => _mapLigne({ ...l, status: l.status || 'active' }));
}
...
async function getGrilleTarifaire() {
  const json = await fetch(API_BASE + '/api/lines/active').then(r => r.json());
  const list = json?.data || json?.lines || [];
  return list.map(l => ({ ...l, depart: l.origin, destination: l.destination, prix: l.price_monthly, prix_mensuel: l.price_monthly, actif: true }));
}
